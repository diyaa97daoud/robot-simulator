...
This section documents the simulation variant evaluated in the archived comparison suite. The purpose of this model is to extend the reference warehouse simulation with decentralized AMR coordination and battery management while making the battery policy more conservative, so that robots are less likely to abandon a pallet or route it through an intermediate area because of low battery. In practical terms, this variant aims to approximate a **no-intermediate-transfer behavior** by forcing earlier recharge decisions.

The analysis below is based on the archived suite outputs of the run, on the implemented simulator logic, and on the automated validation tests available in the project.
...
The analyzed suite does not execute a single scenario only. It performs a controlled comparison over multiple seeds, arrival rates, and fleet sizes.
...
| Communication     | None                      | Broadcast claim/bid exchanges                                           |
...
### 7.2 Decentralized Task Allocation

The optimized mode uses a decentralized bid/claim mechanism:
...
- `pickupCost` is the shortest-path distance from robot to pallet.
- `deliveryCost` is the path cost to the chosen destination.
- `congestionPenalty` penalizes blocked pickup positions.
- `batteryPenalty` penalizes assignments that are risky under the current battery level.
- `queuePenalty` penalizes the use of intermediate areas with occupancy.

### 7.3 Battery and Recharge Policy
...
The principal empirical validation is the 120-run comparison suite summarized in this section. Results were computed from the archived CSV outputs of the experimental batch rather than from manual observation.

## 8. Experimental Results

### 9.1 Reference Model Performance
...
The reference model scales almost linearly with increasing arrival rate over the short 300-step horizon, because it is not constrained by charging, communication, or persistent fleet limitations.

### 9.2 Optimized Variant Performance by Fleet Size
...
### 9.3 Best Optimized Configuration Compared With the Reference
...
### 9.4 Statistical View of the Best Optimized Configuration
...
### 9.5 Evidence Concerning Intermediate-Area Usage
