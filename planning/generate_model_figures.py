...
MODEL_RUNS = {
    ...
    "Model 2": ROOT / "build" / "suite" / "run-1773687001383",
...
def plot_recharge_and_conflicts(stats: dict[str, dict[str, object]]) -> None:
    ...
    for model_name in ("Model 1", "Model 2", "Model 3"):
        recharge_means, _ = best_model_stats(stats[model_name], "recharge_count")
        conflict_means, _ = best_model_stats(stats[model_name], "blocked_conflicts")
        axes[0].bar([x + offsets[model_name] for x in x_positions], recharge_means, width=width, label=model_name, color=MODEL_COLORS[model_name])
    ...
    axes[0].set_title("Recharge Count")
    axes[0].set_xlabel("Arrival rate")
    axes[0].set_ylabel("Mean recharge count")
    ...
    handles, labels = axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", ncol=3, frameon=False)
    fig.suptitle("Operational Cost Across Models", y=1.03)
